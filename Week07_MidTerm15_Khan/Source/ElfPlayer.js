...
.factory('gameEventService', function($rootScope) {
	return {
		...
		towerNameBroadcast: function(message) {
			this.message = message;
			this.broadcastMessage('towerNameBroadcast');
		},
		
		towerHealthBroadcast: function(message) {
			this.message = message;
			this.broadcastMessage('towerHealthBroadcast');
		},
		
		towerHitPointsBroadcast: function(message) {
			this.message = message;
			this.broadcastMessage('towerHitPointsBroadcast');
		},
		
		towerDamageBroadcast: function(message) {
			this.message = message;
			this.broadcastMessage('towerDamageBroadcast');
		},
...
.controller('ElfController', function($scope, $http, gameEventService, elfgame, people) {	
	...
	$scope.$on('towerNameBroadcast', function() {
		$scope.$apply(function() { $scope.towerName = gameEventService.message; });
		console.log(gameEventService.message);
	});
	
	$scope.$on('towerHealthBroadcast', function() {
		$scope.$apply(function() { $scope.towerHealth = gameEventService.message; });
		console.log(gameEventService.message);
	});
	
	$scope.$on('towerHitPointsBroadcast', function() {
		$scope.$apply(function() { $scope.towerHitpoints = gameEventService.message; });
		console.log(gameEventService.message);
	});
	
	$scope.$on('towerDamageBroadcast', function() {
		$scope.$apply(function() { $scope.towerDamage = gameEventService.message; });
		console.log(gameEventService.message);
	});
